...
doDeletion="n"
echo "There are files in /var/lib/salmon and /usr/local/vpnserver that you might"
echo "want to remove. Should I remove those two directories for you? (y/n)"
read doDeletion
...
case "$doDeletion" in
[yY]) 
	rm --verbose -r /var/lib/salmon
	rm --verbose -r /usr/local/vpnserver
	echo "Finished removing /var/lib/salmon and /usr/local/vpnserver."
;;
*)
	echo "Not removing /var/lib/salmon and /usr/local/vpnserver."
;;
esac
